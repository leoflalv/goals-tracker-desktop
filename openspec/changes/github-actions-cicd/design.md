...
- Branch/PR test runs — only tag pushes trigger this workflow (a separate PR check workflow can be added later)
...
### 2. `tauri-apps/tauri-action` as the build driver
...
### 3. Matrix strategy: macOS + Windows only
...
### 5. Draft releases (auto-publish off)
